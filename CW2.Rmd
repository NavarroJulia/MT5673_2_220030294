...
```
This gives us an approximation to the probability `r Prob1` where we used `r num_sim` loops.
...
One can notice that as the number of simulations increases, the values in the above table tend to a certain value (the actual probability `r Prob1`). Thus, the more times the monte carlo simulation for the probability is computed, the more accurate the bootstap approximation becomes.

<br>

```{r, echo = FALSE}

Data.Var <- data.frame(VarResults, NRepeat)
...
(Actually works if repeated enough times, but still check) Change so the win rate increases from ~0 to ~1:
