...
```{r, echo = FALSE}
N_Repeat <- 100 # no. of competitions entered 

pwins <- c()   # empty vector for winning probabilities
...
K[i] <- sample((rand[rand == pw]), size=1) # take one value from rand vector st its the pw value
    ...
    if (sum(length(WINorLOSS[WINorLOSS == 1])) == 7 | sum(length(WINorLOSS[WINorLOSS == 0])) == 3){
      ...
      #plot(Total_G, pw, cex = 1, pch = 3, xlab ="x", ylab ="y", col ="black")
  ...
  #points(Total_G, pw, cex = 2, pch = 6, col ="blue")
}
...
#summary(Total_G)
#
#plot(1:N_Repeat ,Total_G)
#plot(Total_G)
#hist(Total_G)

# Data.Game <- data.frame(1:N_Repeat ,Total_G)

#ggplot(Data.Game) +
#  geom_line(aes(y = Total_G, x = 1:N_Repeat))+
#  geom_hline(yintercept=mean(Total_G), color= "red")+
#  labs(colour = "Total games")+ 
#  xlab("Number of games playes") +
#  ylab("Total games") +
